...
namespace UnrealBuildTool
{
	...
	public abstract partial class TargetRules
	{
		...
		{
			get { return bAllowHotReloadOverride ?? ((Type == TargetType.Editor || Type == TargetType.App) && LinkType == TargetLinkType.Modular); }
		...
		/// </summary>
		public bool bBuildAllPlugins = true;
		...
		{
			get { return (Type == TargetType.Editor || Type == TargetType.App); }
		...
		{
			get { return bBuildWithEditorOnlyDataOverride ?? (Type == TargetType.Editor || Type == TargetType.App || Type == TargetType.Program); }
		...
		public bool bBuildDeveloperTools
		{
			...
			get { return bBuildDeveloperToolsOverride ?? (bCompileAgainstEngine && (Type == TargetType.Editor || Type == TargetType.App|| Type == TargetType.Program || (Configuration != UnrealTargetConfiguration.Test && Configuration != UnrealTargetConfiguration.Shipping))); }
		}
		...
		public bool bCompileSpeedTree
		{
			...
			get { return bOverrideCompileSpeedTree ?? (Type == TargetType.Editor || Type == TargetType.App); }
		}
		...
		{
			get { return bIncludePluginsForTargetPlatformsOverride ?? (Type == TargetType.Editor || Type == TargetType.App); }
		...
		{
			get { return bWithPerfCountersOverride ?? (Type == TargetType.Editor || Type == TargetType.App || Type == TargetType.Server); }
		...
		{
			get { return bBuildAdditionalConsoleAppOverride ?? (Type == TargetType.Editor || Type == TargetType.App); }
		...
		public TargetLinkType LinkType
		{
			get
			{
				return (LinkTypePrivate != TargetLinkType.Default) ? LinkTypePrivate : ((Type == global::UnrealBuildTool.TargetType.Editor  ||  Type == global::UnrealBuildTool.TargetType.App) ? TargetLinkType.Modular : TargetLinkType.Monolithic);
			}
			set
			{
		...
		public string? ModuleNameExtra
		{
			get 
			{
				return ModuleNameExtraPrivate == null ? "" : ModuleNameExtraPrivate;
			}
			set
			{
				ModuleNameExtraPrivate = value;
			}
		}

		private string? ModuleNameExtraPrivate;
		/// <summary>
...
#nullable restore
		...
		internal void SetOverridesForTargetType()
		{
			...
			else if (Type == global::UnrealBuildTool.TargetType.App)
			{
				GlobalDefinitions.Add("UE_EDITOR=1");
				//GlobalDefinitions.Add("UE_APP=1");
			}
		}
		...
		internal UnrealTargetPlatform[] GetSupportedPlatforms()
		{
			...
			else if (Type == TargetType.Editor || Type == TargetType.App) 
			{
		...
		internal UnrealTargetConfiguration[] GetSupportedConfigurations()
		{
			...
			// Otherwise, get the default for the target type
			if (Type == TargetType.Editor || Type == TargetType.App)
			{
	...
	public partial class ReadOnlyTargetRules
	{
		...
		public string? ModuleNameExtra
		{
			get { return Inner.ModuleNameExtra; }
		}
