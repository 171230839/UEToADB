	...
	class VCProjectFileGenerator : ProjectFileGenerator
	{
		...
		protected override bool WriteProjectFiles(PlatformProjectGeneratorCollection PlatformProjectGenerators)
		{
			...
			if (AutomationProjectFiles.Any())
			{
				...
				DirectoryReference AutomationToolDir = DirectoryReference.Combine(UnrealBuildTool.EngineSourceProgramsDirectory, "AutomationTool");
				new XDocument(
		...
		protected override bool WriteMasterProjectFile(ProjectFile UBTProject, PlatformProjectGeneratorCollection PlatformProjectGenerators)
		{
			...
			if (bSuccess && Settings.bWriteSolutionOptionFile)
			{
				...
				if (!FileReference.Exists(SolutionOptionsFileName))
				{
					...
					VCSolutionConfigCombination DefaultConfig = SolutionConfigCombinations.Find(x => x.Configuration == UnrealTargetConfiguration.Development && x.Platform == UnrealTargetPlatform.Win64 && (x.TargetConfigurationName == TargetType.Editor || x.TargetConfigurationName == TargetType.App));
